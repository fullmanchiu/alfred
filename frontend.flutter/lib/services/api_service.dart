...
class ApiService {
  ...
  static Future<Map<String, dynamic>> getUserProfile() async {
    ...
    if (response.statusCode == 200) {
      ...
      final errorData = jsonDecode(response.body);
      throw errorData['message'] ?? errorData['detail'] ?? '获取用户信息失败';
    }
  }
  ...
  static Future<Map<String, dynamic>> getTransactions({
    ...
    if (response.statusCode == 200) {
      ...
      final errorData = jsonDecode(response.body);
      throw errorData['message'] ?? errorData['detail'] ?? '获取记账记录失败';
    }
  }
  ...
  static Future<Map<String, dynamic>> createTransaction(Map<String, dynamic> transactionData) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      final errorData = jsonDecode(response.body);
      throw errorData['message'] ?? errorData['detail'] ?? '创建记账记录失败';
    }
  }
  ...
  static Future<Map<String, dynamic>> getCategories({String? type}) async {
    ...
    if (response.statusCode == 200) {
      ...
      final errorData = jsonDecode(response.body);
      throw errorData['message'] ?? errorData['detail'] ?? '获取分类列表失败';
    }
  }
  ...
  // 初始化默认分类
  static Future<Map<String, dynamic>> initDefaultCategories() async {
    final response = await http.post(
      Uri.parse('$baseUrl/api/v1/categories/init-defaults'),
      headers: await getHeaders(),
    );

    print('API Request: POST $baseUrl/api/v1/categories/init-defaults');
    print('Response Status: ${response.statusCode}');
    ...
    throw errorData['message'] ?? '初始化默认分类失败';
  }

  // 同步系统分类（版本控制）
  static Future<Map<String, dynamic>> syncSystemCategories() async {
    final response = await http.post(
      Uri.parse('$baseUrl/api/v1/categories/sync-system'),
      headers: await getHeaders(),
    );

    if (_debugMode) {
      print('API Request: POST $baseUrl/api/v1/categories/sync-system');
      print('Response Status: ${response.statusCode}');
    }
    ...
    final errorData = jsonDecode(utf8.decode(response.bodyBytes));
    throw errorData['message'] ?? '同步系统分类失败';
  }

  // 重置用户数据
  static Future<Map<String, dynamic>> resetUserData() async {
    final response = await http.post(
      Uri.parse('$baseUrl/api/v1/users/reset-data'),
      headers: await getHeaders(),
    );

    if (_debugMode) {
      print('API Request: POST $baseUrl/api/v1/users/reset-data');
      print('Response Status: ${response.statusCode}');
    }
    ...
    if (response.statusCode == 200) {
      ...
      return {
        ...
        'deletedBudgets': data['data']['deletedBudgets'],
        'deletedCategories': data['data']['deletedCategories'],
        'initializedCategories': data['data']['initializedCategories'],
      };
    }
    ...
    final errorData = jsonDecode(utf8.decode(response.bodyBytes));
    throw errorData['message'] ?? '重置数据失败';
  }

  // 恢复被软删除的系统分类
  static Future<Map<String, dynamic>> restoreSystemCategories() async {
    final response = await http.post(
      Uri.parse('$baseUrl/api/v1/user-data/restore-system-categories'),
      headers: await getHeaders(),
    );

    if (_debugMode) {
      print('API Request: POST $baseUrl/api/v1/user-data/restore-system-categories');
      print('Response Status: ${response.statusCode}');
    }
    ...
    final errorData = jsonDecode(utf8.decode(response.bodyBytes));
    throw errorData['message'] ?? '恢复系统分类失败';
  }

  // 重新初始化用户的默认分类
  static Future<Map<String, dynamic>> resetCategories() async {
    final response = await http.post(
      Uri.parse('$baseUrl/api/v1/user-data/reset-categories'),
      headers: await getHeaders(),
    );

    if (_debugMode) {
      print('API Request: POST $baseUrl/api/v1/user-data/reset-categories');
      print('Response Status: ${response.statusCode}');
    }
    ...
    final errorData = jsonDecode(utf8.decode(response.bodyBytes));
    throw errorData['message'] ?? '重置分类失败';
  }

  // 强制重新同步系统分类（忽略版本号检查）
  static Future<Map<String, dynamic>> forceSyncCategories() async {
    final response = await http.post(
      Uri.parse('$baseUrl/api/v1/user-data/force-sync-categories'),
      headers: await getHeaders(),
    );

    if (_debugMode) {
      print('API Request: POST $baseUrl/api/v1/user-data/force-sync-categories');
      print('Response Status: ${response.statusCode}');
    }
    ...
    final errorData = jsonDecode(utf8.decode(response.bodyBytes));
    throw errorData['message'] ?? '强制同步分类失败';
  }
  ...
  static Future<Map<String, dynamic>> getBudgets({String? period}) async {
    ...
    if (response.statusCode == 200) {
      ...
      final errorData = jsonDecode(response.body);
      throw errorData['message'] ?? errorData['detail'] ?? '获取预算列表失败';
    }
  }
  ...
  static Future<Map<String, dynamic>> getOverviewStatistics({
    String? period,
    String? startDate,
    String? endDate,
  }) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Get statistics failed: ${response.statusCode}');
    }
  }
  ...
  /// 获取预算建议
  static Future<Map<String, dynamic>> getBudgetAdvice({
    String period = 'monthly',
  }) async {
    final headers = await getHeaders();
    final queryParams = period.isNotEmpty ? '?period=$period' : '';
    final uri = Uri.parse('$baseUrl/api/v1/llm/budget/advice$queryParams');
    ...
    if (response.statusCode == 200) {
      ...
      final errorData = jsonDecode(response.body);
      throw errorData['message'] ?? '获取预算建议失败';
    }
  }

  /// 分析消费行为
  static Future<Map<String, dynamic>> analyzeSpending(
    Map<String, dynamic> requestData,
  ) async {
    final headers = await getHeaders();
    final uri = Uri.parse('$baseUrl/api/v1/llm/spending/analyze');

    // LLM调用可能需要较长时间，使用150秒超时
    final response = await http.post(
      uri,
      headers: headers,
      ...
      onTimeout: () => throw '请求超时，AI分析耗时过长，请稍后重试',
    );
    ...
    if (response.statusCode == 200) {
      ...
      final errorData = jsonDecode(response.body);
      throw errorData['message'] ?? '消费分析失败';
    }
  }

  /// 流式分析消费行为（使用 SSE）
  static Stream<String> analyzeSpendingStream(
    Map<String, dynamic> requestData,
  ) async* {
    final headers = await getHeaders();
    final uri = Uri.parse('$baseUrl/api/v1/llm/spending/analyze-stream');
  ...
  static Future<Map<String, dynamic>> generateFinancialReport(
    ...
    final headers = await getHeaders();
    final uri = Uri.parse('$baseUrl/api/v1/llm/financial/report');
    ...
    if (response.statusCode == 200) {
      ...
      final errorData = jsonDecode(response.body);
      throw errorData['message'] ?? '生成财务报告失败';
    }
  }
}