...
class CategoryController(
    ...
    fun updateCategory(
        ...
        val updatedCategory = existingCategory.copy(
            ...
            parentId = request.parentId ?: existingCategory.parentId,
    ...
    fun initDefaultCategories(
        ...
        return ResponseEntity.ok(mapOf(
            "initialized" to true,
            "message" to "成功初始化 ${categories.size} 个分类",
    ...
    fun syncSystemCategories(
        authentication: Authentication
    ): ResponseEntity<Map<String, Any>> {
        val userId = authService.getCurrentUserId(authentication)
        val synced = categoryService.syncSystemCategories(userId)

        return ResponseEntity.ok(mapOf(
            "synced" to synced,
            "message" to if (synced) "系统分类已同步" else "系统分类已是最新版本"
        ))
    }
}
