...
class AuthService(
    ...
    fun getUserById(userId: Long): User {
        return userRepository.findById(userId)
            .orElseThrow { apiException(ErrorCode.USER_NOT_FOUND, userId.toString()) }
    }

    @Transactional
    fun updateUserProfile(userId: Long, profileData: Map<String, Any?>): User {
        val user = getUserById(userId)

        // 只更新允许的字段
        val updatedUser = user.copy(
            email = profileData["email"] as? String ?: user.email,
            nickname = profileData["nickname"] as? String ?: user.nickname
        )
