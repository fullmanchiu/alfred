...
var _useCursor = _interopRequireDefault(require("../hooks/use-cursor"));
var _useInputHeight = _interopRequireDefault(require("../hooks/use-input-height"));
var _useSlotBuilder = _interopRequireDefault(require("../hooks/use-slot-builder"));
var _useSlotConfigState = _interopRequireDefault(require("../hooks/use-slot-config-state"));
var _Skill = _interopRequireDefault(require("./Skill"));
const SlotTextArea = /*#__PURE__*/_react.default.forwardRef((_, ref) => {
  const {
    onChange,
    onKeyUp,
    onKeyDown,
  ...
  const slotDomMap = (0, _react.useRef)(new Map());
  const isCompositionRef = (0, _react.useRef)(false);
  const keyLockRef = (0, _react.useRef)(false);
  const lastSelectionRef = (0, _react.useRef)(null);
  const skillDomRef = (0, _react.useRef)(null);
  const skillRef = (0, _react.useRef)(null);
  ...
  const inputHeightStyle = (0, _useInputHeight.default)(mergeStyle, autoSize, editableRef);
  ...
  const [slotConfigMap, {
    ...
    mergeSlotConfig,
    getNodeTextValue
  }] = (0, _useSlotConfigState.default)(slotConfig);
  const [slotPlaceholders, setSlotPlaceholders] = (0, _react.useState)(new Map());
  const [skillPlaceholders, setSkillPlaceholders] = (0, _react.useState)(null);
  // ============================ Cursor =============================
  const {
    setEndCursor,
    setStartCursor,
    ...
    copySelectionString,
    getCleanedText
  } = (0, _useCursor.default)({
    prefixCls,
    getSlotDom: key => slotDomMap.current.get(key),
    slotConfigMap,
    getNodeInfo,
  ...
  const renderSlot = (config, slotSpan) => {
      ...
      switch (config.type) {
        ...
        case 'input':
          return /*#__PURE__*/_react.default.createElement(_antd.Input, {
            ...
            onChange: e => {
              updateSlot(config.key, e.target.value, e);
            },
            spellCheck: false
          });
        case 'select':
          return /*#__PURE__*/_react.default.createElement(_antd.Dropdown, {
            disabled: readOnly,
            menu: {
              items: config.props?.options?.map(opt => ({
                label: opt,
                key: opt
              })),
              ...
              onSelect: ({
                ...
                updateSlot(config.key, key, domEvent);
              }
            },
            trigger: ['click']
          }, /*#__PURE__*/_react.default.createElement("span", {
            className: (0, _clsx.clsx)(`${prefixCls}-slot-select`, {
              placeholder: !value,
          ...
          }, /*#__PURE__*/_react.default.createElement(_icons.CaretDownFilled, null))));
        case 'tag':
          return /*#__PURE__*/_react.default.createElement("span", {
            className: `${prefixCls}-slot-tag`
          }, config.props?.label || config.props?.value || '');
        case 'custom':
          return config.customRender?.(value, value => {
            updateSlot(config.key, value);
          }, {
            disabled,
            readOnly
          }, config);
    ...
    return /*#__PURE__*/(0, _reactDom.createPortal)(renderContent(), slotSpan);
  };
  const getSlotListNode = slotConfig => {
    return slotConfig.reduce((nodeList, config) => {
      ...
      (0, _warning.default)(!!slotKey, 'Sender', `Slot key is missing: ${slotKey}`);
      if (slotKey) {
        let slotSpan;
        let slotDom;
  ...
  const insertSkill = () => {
    if (skill && skillRef.current !== skill) {
      ...
      const reactNode = /*#__PURE__*/(0, _reactDom.createPortal)( /*#__PURE__*/_react.default.createElement(_Skill.default, (0, _extends2.default)({
  ...
  const initRenderSlot = () => {
    if (slotConfig && slotConfig.length > 0 && editableRef.current) {
      initClear();
      appendNodeList(getSlotListNode(slotConfig));
    }
  };

  // 检查是否应该跳过键盘事件处理
  const shouldSkipKeyHandling = e => {
    const eventRes = onKeyDown?.(e);
    return keyLockRef.current || isCompositionRef.current || eventRes === false;
  };

  // 处理删除操作（退格键、剪切等）
  const handleDeleteOperation = (e, operationType) => {
    ...
    if (operationType === 'backspace' && focusOffset === 0) {
      ...
      if (previousSibling) {
        const nodeInfo = getNodeInfo(previousSibling);
        if (nodeInfo) {
          const {
            slotKey,
            skillKey
  ...
  const shouldSubmitForm = e => {
    ...
    return submitType === 'enter' && !shiftKey && !isModifierPressed || submitType === 'shiftEnter' && shiftKey && !isModifierPressed;
  };

  //  处理skill区域的键盘事件
  const handleSkillAreaKeyEvent = () => {
    if (!skillDomRef.current || !editableRef.current || skillDomRef.current.getAttribute('contenteditable') === 'false') {
      return;
    }
    const selection = getSelection();
    if (!selection?.anchorNode || !skillDomRef.current.contains(selection.anchorNode) || !editableRef.current.contains(selection.anchorNode)) return;
    skillDomRef.current.setAttribute('contenteditable', 'false');
    skillDomRef.current.classList.remove(`${prefixCls}-skill-empty`);
    focus({
      cursor: 'end'
  ...
  const onInternalKeyDown = e => {
    ...
    // 确保事件目标是可编辑区域
    if (e.target !== editableRef.current && !editableRef.current?.contains(e.target)) {
      return;
    }
  ...
  const onInternalPaste = e => {
    ...
    if (text) {
      ...
      try {
        ...
        (0, _warning.default)(false, 'Sender', `insertText command failed: ${err}`);
      }
      if (!success) {
        insert([{
          type: 'text',
  ...
  const onInternalSelect = () => {
    const editableDom = editableRef.current;
    const selection = getSelection();
    if (editableDom && selection?.focusNode === editableDom && selection.focusOffset === 0 && getEditorValue().skill) {
  ...
  const insert = (slotConfig, position, replaceCharacters, preventScroll) => {
    const editableDom = editableRef.current;
    if (!editableDom) return;
    try {
      // 合并配置并生成节点
      ...
      (0, _warning.default)(false, 'Sender', `Insert operation failed: ${error}`);
    }
  };

  // 获取插入上下文信息
  const getInsertContext = (position, editableDom) => {
    const {
      type,
      slotKey,
      slotType,
  ...
  // 处理字符替换
  const handleCharacterReplacement = (range, replaceCharacters, editableDom) => {
    const {
    ...
    if (cursorPosition >= replaceCharacters.length && textBeforeCursor.endsWith(replaceCharacters) && startContainer && startOffset >= 0) {
   ...
   * 根据位置插入节点
   */
  const insertNodesWithPosition = (slotNodes, range, context) => {
    const {
      type,
      slotKey,
      slotType
    ...
    slotNodes.forEach(node => {
      // 处理slot插入的特殊逻辑
      if (shouldSkipFirstNode && type === 'slot' && (slotType !== 'content' || node.nodeType !== Node.TEXT_NODE) && slotKey) {
        shouldSkipFirstNode = false;
        const lastDom = getSlotLastDom(slotKey);
        if (lastDom) {
          range.setStartAfter(lastDom);
        }
      }
  ...
  // 完成插入操作，设置光标并触发更新
  const finalizeInsertion = (slotNodes, range, selection, preventScroll) => {
  ...
  const focus = options => {
    ...
    switch (mergeOptions.cursor) {
      case 'slot':
        setSlotFocus(editableRef, options?.key, mergeOptions.preventScroll);
        break;
      case 'start':
  ...
  // ============================ Render =============================
  return /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, /*#__PURE__*/_react.default.createElement("div", (0, _extends2.default)({}, inputProps, {
    role: "textbox",
    tabIndex: 0,
    style: {
      ...mergeStyle,
    ...
    className: (0, _clsx.clsx)(inputCls, `${inputCls}-slot`, contextConfig.classNames.input, classNames.input, {
  ...
  }, Array.from(slotPlaceholders.values()), skillPlaceholders));
});
